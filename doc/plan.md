...
- [x] Crear Google Sheet con hojas `Productos`, `Ventas`, `Sesiones` (columnas exactas de `base.md` §2). *Hecho — flujo completo probado en verde por el dueño 23/09/2026 (login, productos, ventas, caja).*
- [x] Script Properties del proyecto: `SPREADSHEET_ID` (id del Sheet — el script es standalone), `USUARIO` y `CLAVE` (o hash) — nunca en el código ni en el Sheet. *Hecho — confirmado por el dueño 23/09/2026 (login real funciona).*
...
- [x] Correr el seed de DummyJSON una vez (`node scripts/seed-dummyjson.mjs > seed-productos.csv` → pegar en la hoja). *Hecho — 194 productos en venta, confirmado por el dueño 23/09/2026.*
- [x] Primer login real desde la app (paso 8 de `extras.md` §3). *Hecho — confirmado por el dueño 23/09/2026.*
