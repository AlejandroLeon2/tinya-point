...
// derived from the snapshot when present and from total / (1 + TAX_RATE)
// (inverse of calcTotal) otherwise. Hooks are data-* scoped to
...
import { calcTax, TAX_RATE } from '../../utils/tax';
...
if (root) {
  ...
  if (!venta) {
    ...
    if (igvEl) {
      const igv = subtotal !== null ? calcTax(subtotal) : venta.total - venta.total / (1 + TAX_RATE);
